...
public class Cuadricula : MonoBehaviour
{
	...
	*/
	...
	void Awake () 
	{
		...
		Turnos = this.GetComponent<Turno>();
	...
	void CrearCelda (int x, int y, int i) 
	{
		...
		etiqueta.rectTransform.SetParent(celda.transform, false); // Padre de la Celda
		etiqueta.text = y.ToString() + " " + x.ToString();

	}

	void JugadorBlancasNegras()
	{
		...
		foreach(Jugador jugadores in Turnos.Jugadores)
    	{
			...
			if(listemp[i] == 1)
			{
				...
				jugadores.miColor = ColorJugador.Blancas;
				
			}
			else if(listemp[i] == 2)
			{

				jugadores.miColor = ColorJugador.Negras;
		...
		foreach (string unidad in unidadsemilla)
		{
			...
			if(unidad.Split('*')[3]=="1" && UnidadScript.UnidadJugador.miColor == ColorJugador.Blancas)
			{
				...
				if(UnidadScript.GetTipoUnidad()==TipoUnidad.Reina)
				{
					...
					UnidadScript.SetCelda("Celda " + unidad.Split('*')[0] + " " + celdabl);
				}
				else if(UnidadScript.GetTipoUnidad()==TipoUnidad.Rey)
				{
					int celdabl = int.Parse(unidad.Split('*')[1])+1;
					UnidadScript.SetCelda("Celda " + unidad.Split('*')[0] + " " + celdabl);
				}
			...
			else
			{
				...
				//if(unidad.Split('*')[3]=="2")
				if(UnidadScript.UnidadJugador.miColor == ColorJugador.Blancas)
				{
				...
				if(unidad.Split('*')[3]=="2")
				{
					...
					if(UnidadScript.GetTipoUnidad()==TipoUnidad.Peon) // Los peones ??¿¿ QUE ESTAN ARRIBA ??¿¿ tienen que moverse S y atacar SE SO
					{
